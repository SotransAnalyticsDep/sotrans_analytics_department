...
class Datamart:
    ...
    # ##################################################
    def calc_start_date(
        self, period: Optional[int] = None, end_date: Optional[dt.date] = None
    ) -> dt.date:
        """
        Рассчитывает начальную дату анализируемого диапазона на основе
        заданного периода.

        Args:
            period (int, необязательный): Количество месяцев, которое нужно
                вычесть из конечной даты. Если None, используется период
                по умолчанию (__period). По умолчанию None.
            end_date (dt.date): Конечная дата анализируемого периода.

        Return:
            dt.date: Рассчитанная начальная дата, установленная на первый
                день получившегося месяца.

        Notes:
            Расчет преобразует конечную дату (self.__end_date) в общее
            количество месяцев, вычитает указанный период, а затем преобразует
            результат обратно в год и месяц. Если получившийся месяц равен 0,
            он корректируется до декабря предыдущего года.
        """
        # ЛОГИРОВАНИЕ
        logger.trace(f"{period=};")
        logger.trace(f"{end_date};")

        # Если period не установлен, значение берётся из экземпляра класса Datamart
        if period is None:
            period: int = self.__period
            logger.debug(
                f"Период не задан, используется значение из Datamart: {self.__period}"
            )
        ...
        if end_date is None:
            ...
            logger.debug(
                f"Конечная дата анализируемого периода не задана, используется значение из Datamart: {self.__end_date}"
            )

        # Расчёт общего количества месяцев и преобразование в год и месяц
        ...
        if start_date_month == 0:
            ...
            start_date_year -= 1
            logger.debug(
                f"Месяц скорректирован до декабря предыдущего года: {start_date_year}.{start_date_month}"
            )

        # Формирование начальной даты анализируемого периода
        start_date: dt.date = dt.date(
            year=start_date_year, month=start_date_month, day=1
        )
        logger.success(
            f"Начальная дата анализируемого периода успешно рассчитана: {start_date}"
        )
